...
export async function generateOnboardingRecommendations(
  ...
  console.log(
    "🚀 Starting generateOnboardingRecommendations for user:",
    userId,
  );
  const { supabase } = createClient(request);

  // Get user's clothing items with category relationships
  console.log("📦 Fetching user clothing items...");
  ...
  console.log(`📈 Found ${items?.length || 0} clothing items`);
  console.log(
    "👕 Clothing items:",
    items?.map((item) => ({
      id: item.id,
      name: item.name,
      category: item.clothing_categories?.name || item.category,
      primary_color: item.primary_color,
    })),
  );

  if (!items || items.length < 2) {
    console.log("⚠️ Insufficient items for recommendations");
  ...
  const prompt =
    `You are an expert fashion stylist creating FIRST-TIME outfit recommendations for a new user. This is their onboarding experience, so focus on:
...
- Color Preferences: ${
      profile?.color_preferences?.join(", ") || "open to suggestions"
    }
...
- weather_appropriateness_score: Seasonal/weather suitability`;
  ...
  try {
    ...
    console.log(
      "✅ Gemini API key found, making request...",
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        return {
          error:
            "We're generating lots of recommendations right now. Please wait 2-3 minutes and try again.",
        };
      }
    ...
    const recommendationsText = result.candidates?.[0]?.content?.parts?.[0]
      ?.text;
    ...
    const validRecommendations = (recommendations as OnboardingRecommendation[])
      .filter((rec) => {
        const hasIds = rec.clothing_item_ids &&
          rec.clothing_item_ids.length >= 2 &&
          rec.clothing_item_ids.length <= 3;
        const validIds = rec.clothing_item_ids?.every((id) =>
    ...
    console.log(
      `✅ Generated ${validRecommendations.length}/${recommendations.length} valid recommendations`,
    );
...
export async function generateOnboardingGaps(userId: string, request: Request) {
  ...
  const categories = [
    ...new Set(
      items.map((item) => (item as any).categories?.name).filter(Boolean),
    ),
  ];
  const colors = [...new Set(items.map((item) => item.primary_color))];

  const prompt =
    `Analyze this BEGINNER wardrobe and suggest 2-3 key pieces that would dramatically improve outfit options.
...
export async function generateOutfitRecommendations(
  ...
  const { data: recentRecs } = options?.refresh
    ? await supabase
      .from("outfit_recommendations")
  ...
  const prompt =
    `You are an expert fashion stylist creating personalized outfit recommendations.

${
      options?.surprise
        ? "SURPRISE MODE: Focus heavily on user profile and past preferences to create delightful, personalized recommendations that feel like you truly know their style."
        : ""
    }
...
${
      JSON.stringify(
        trends?.map((t) => ({
          colors: t.trending_colors,
          styles: t.trending_styles,
        })) || [],
        null,
        2,
      )
...
${
      options?.refresh
        ? `Recent Combinations to Avoid (generate different ones):
...
${
      options?.refresh
        ? "✓ Generate DIFFERENT combinations than recent ones listed above"
        : ""
    }
${
      options?.surprise
        ? "✓ SURPRISE MODE: Leverage their style preferences, color preferences, and past interactions heavily"
        : ""
    }
${
      options?.surprise
        ? "✓ Create outfits that feel personally curated based on their profile data"
        : ""
    }
${
      options?.surprise
        ? "✓ Show you understand their style by referencing their preferences in recommendations"
        : ""
    }
...
[
  ...
  try {
    ...
    const validRecommendations = (recommendations as AdvancedRecommendation[])
      .filter((rec) =>
        rec.clothing_item_ids &&
        rec.clothing_item_ids.length >= 2 &&
        rec.clothing_item_ids.every((id) =>
          items.find((item) => item.id === id)
        )
      );
    ...
    for (const rec of validRecommendations) {
      const { data: saved } = await supabase
        ...
        .insert({
          ...
          based_on_past_preferences: pastInteractions &&
      ...
      if (saved) {
        savedRecommendations.push({
          ...
          items: rec.clothing_item_ids.map((id) =>
            items.find((item) => item.id === id)
          ).filter(Boolean),
        });
      }
...
export async function analyzeWardrobeGaps(userId: string, request: Request) {
  ...
  const prompt =
    `Analyze this comprehensive wardrobe and identify strategic gaps for a sophisticated user.
    ...
    for (const gap of gaps.slice(0, 8)) {
      await supabase
        ...
        .upsert({
          ...
          priority: gap.priority === "critical"
            ? 10
            : gap.priority === "high"
            ? 8
            : gap.priority === "medium"
          ...
          estimated_cost: gap.investment_level === "high"
            ? 200
            : gap.investment_level === "medium"
            ? 100
            : 50,
        });
    }
