...
INSERT INTO
    public.seasons (name, display_name, display_order)
VALUES
    ('spring', 'Spring', 1),
    ('summer', 'Summer', 2),
    ('fall', 'Fall', 3),
...
INSERT INTO
    public.fit_preferences (name, display_name, display_order)
VALUES
    ('tight', 'Tight', 1),
    ('fitted', 'Fitted', 2),
    ('regular', 'Regular', 3),
...
ALTER TABLE
    public.clothing_items DROP COLUMN season,
    DROP COLUMN fit;
...
ADD
    COLUMN preferred_fit_name TEXT DEFAULT 'regular';

UPDATE
    public.user_profiles
SET
    preferred_fit_name = preferred_fit :: text;

ALTER TABLE
    public.user_profiles DROP COLUMN preferred_fit;

-- Create RPC functions for season management
CREATE
OR REPLACE FUNCTION find_or_create_season(season_name TEXT) RETURNS UUID AS $$ DECLARE season_id UUID;

clean_name TEXT;

BEGIN -- Clean and normalize the season name
...
INSERT INTO
    public.seasons (name, display_name)
VALUES
    (
        clean_name,
        initcap(replace(clean_name, '_', ' '))
...
CREATE
OR REPLACE FUNCTION find_or_create_fit_preference(fit_name TEXT) RETURNS UUID AS $$ DECLARE fit_id UUID;

clean_name TEXT;

BEGIN -- Clean and normalize the fit name
...
INSERT INTO
    public.fit_preferences (name, display_name)
VALUES
    (clean_name, initcap(clean_name)) RETURNING id INTO fit_id;
...
-- Enable RLS
ALTER TABLE
    public.seasons ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    public.fit_preferences ENABLE ROW LEVEL SECURITY;

-- Public read access for reference tables
CREATE POLICY "Anyone can view seasons" ON public.seasons FOR
SELECT
    USING (true);

CREATE POLICY "Anyone can view fit preferences" ON public.fit_preferences FOR
SELECT
    USING (true);
...
ALTER TABLE
    public.seasonal_trends DROP COLUMN season;
...
-- Drop old enum column
ALTER TABLE
    public.outfit_recommendations DROP COLUMN season;

-- Update function signature to use TEXT instead of season enum
CREATE
OR REPLACE FUNCTION get_trending_categories_for_season(target_season TEXT, target_year INTEGER) RETURNS JSON AS $$ BEGIN RETURN (
    SELECT
        json_agg(
            json_build_object(
                'id',
