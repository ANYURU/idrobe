...
BEGIN
    -- Check if waitlist_archive table exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'waitlist_archive') THEN
        RETURN FALSE;
    END IF;
...
CREATE INDEX IF NOT EXISTS idx_duplicates_unresolved ON public.clothing_duplicates(user_id) WHERE user_confirmed IS NULL;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'wardrobe_gaps' AND policyname = 'Users can manage own wardrobe gaps') THEN
        CREATE POLICY "Users can manage own wardrobe gaps" ON public.wardrobe_gaps 
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'outfit_collections' AND policyname = 'Users can manage own collections') THEN
        CREATE POLICY "Users can manage own collections" ON public.outfit_collections 
        FOR ALL USING ((select auth.uid()) = user_id);
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'user_interactions' AND policyname = 'Users can manage own interactions') THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'event_templates' AND policyname = 'Users can manage own event templates') THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'seasonal_trend_categories' AND policyname = 'Anyone can view trend categories') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_wardrobe_gaps_updated_at') THEN
        CREATE TRIGGER update_wardrobe_gaps_updated_at 
        BEFORE UPDATE ON public.wardrobe_gaps 
        FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_outfit_collections_updated_at') THEN
