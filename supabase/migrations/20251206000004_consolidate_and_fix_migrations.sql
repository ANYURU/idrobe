...
DO $$ BEGIN -- Add subscription-related columns
IF NOT EXISTS (
    SELECT
        1
    FROM
...
ADD
    COLUMN current_plan_id UUID REFERENCES public.subscription_plans(id);
...
ALTER TABLE
    public.user_profiles
ADD
    COLUMN subscription_status TEXT DEFAULT 'free' CHECK (
...
ALTER TABLE
    public.user_profiles
ADD
    COLUMN plan_expires_at TIMESTAMPTZ;
...
IF NOT EXISTS (
    ...
    WHERE
        table_name = 'user_profiles'
        AND column_name = 'monthly_uploads_used'
) THEN
ALTER TABLE
    public.user_profiles
ADD
    COLUMN monthly_uploads_used INTEGER DEFAULT 0;
...
IF NOT EXISTS (
    ...
    WHERE
        table_name = 'user_profiles'
        AND column_name = 'monthly_recs_used'
) THEN
ALTER TABLE
    public.user_profiles
ADD
    COLUMN monthly_recs_used INTEGER DEFAULT 0;
...
ALTER TABLE
    public.user_profiles
ADD
    COLUMN usage_reset_date TIMESTAMPTZ DEFAULT date_trunc('month', NOW());
...
ALTER TABLE
    public.user_profiles
ADD
    COLUMN preferred_currency TEXT DEFAULT 'USD';
...
ALTER TABLE
    public.user_profiles
ADD
    COLUMN billing_currency TEXT DEFAULT 'USD';
...
DO $$ BEGIN -- Add foreign key columns
IF NOT EXISTS (
    SELECT
        1
    FROM
...
ADD
    COLUMN category_id UUID REFERENCES public.clothing_categories(id);
...
ALTER TABLE
    public.clothing_items
ADD
    COLUMN subcategory_id UUID REFERENCES public.clothing_subcategories(id);
...
IF NOT EXISTS (
    ...
    WHERE
        table_name = 'clothing_items'
        AND column_name = 'season_names'
) THEN
ALTER TABLE
    public.clothing_items
ADD
    COLUMN season_names TEXT [] DEFAULT '{}';
...
IF NOT EXISTS (
    ...
    WHERE
        table_name = 'clothing_items'
        AND column_name = 'fit_name'
) THEN
ALTER TABLE
    public.clothing_items
ADD
    COLUMN fit_name TEXT DEFAULT 'regular';
...
IF NOT EXISTS (
    ...
    WHERE
        table_name = 'clothing_items'
        AND column_name = 'weather_suitable_names'
) THEN
ALTER TABLE
    public.clothing_items
ADD
    COLUMN weather_suitable_names TEXT [] DEFAULT '{}';
...
ALTER TABLE
    public.clothing_items
ADD
    COLUMN sustainability_score DECIMAL(3, 2) CHECK (
...
CREATE
OR REPLACE FUNCTION public.create_user_profile() RETURNS TRIGGER
SET
...
CREATE
OR REPLACE FUNCTION public.find_or_create_category(category_name TEXT) RETURNS UUID
SET
    search_path = '' AS $$ DECLARE category_id UUID;
...
BEGIN IF category_name IS NULL
...
VALUES
    (
        ...
        is_common_category,
        CASE
            WHEN is_common_category THEN 'system'
            ELSE 'ai_suggested'
        END
...
CREATE
OR REPLACE FUNCTION public.find_or_create_subcategory(subcategory_name TEXT, parent_category_id UUID) RETURNS UUID
SET
    search_path = '' AS $$ DECLARE subcategory_id UUID;
...
CREATE
OR REPLACE FUNCTION public.validate_currency_code() RETURNS TRIGGER
SET
    search_path = '' AS $$ BEGIN IF NEW.preferred_currency IS NOT NULL
...
) THEN CREATE INDEX IF NOT EXISTS idx_clothing_embedding ON public.clothing_items USING ivfflat(embedding vector_cosine_ops) WITH (lists = 100);
...
ALTER TABLE
    public.clothing_item_style_tags ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE
    public.clothing_categories ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    public.clothing_subcategories ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    public.style_tags ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    public.subscription_plans ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    public.supported_currencies ENABLE ROW LEVEL SECURITY;
...
) THEN CREATE POLICY "Users can view own profile" ON public.user_profiles FOR
SELECT
    USING (
        (
            select
...
) THEN CREATE POLICY "Users can insert own profile" ON public.user_profiles FOR
INSERT
    WITH CHECK (
        (
            select
...
) THEN CREATE POLICY "Users can update own profile" ON public.user_profiles FOR
UPDATE
    USING (
        (
            select
...
DO $$ BEGIN IF NOT EXISTS (
    ...
    WHERE
        ...
        AND policyname = 'Users can view own clothing items'
) THEN CREATE POLICY "Users can view own clothing items" ON public.clothing_items FOR
SELECT
    USING (
        (
            select
...
IF NOT EXISTS (
    ...
    WHERE
        tablename = 'clothing_items'
        AND policyname = 'Users can insert own clothing items'
) THEN CREATE POLICY "Users can insert own clothing items" ON public.clothing_items FOR
INSERT
    WITH CHECK (
        (
            select
...
IF NOT EXISTS (
    ...
    WHERE
        tablename = 'clothing_items'
        AND policyname = 'Users can update own clothing items'
) THEN CREATE POLICY "Users can update own clothing items" ON public.clothing_items FOR
UPDATE
    USING (
        (
            select
...
) THEN CREATE POLICY "Anyone can view active categories" ON public.clothing_categories FOR
SELECT
...
IF NOT EXISTS (
    ...
    FROM
        pg_policies
    WHERE
        tablename = 'clothing_subcategories'
        AND policyname = 'Anyone can view active subcategories'
) THEN CREATE POLICY "Anyone can view active subcategories" ON public.clothing_subcategories FOR
SELECT
    USING (is_active = TRUE);
...
IF NOT EXISTS (
    ...
    WHERE
        ...
        AND policyname = 'Anyone can view active style tags'
) THEN CREATE POLICY "Anyone can view active style tags" ON public.style_tags FOR
SELECT
    USING (is_active = TRUE);
...
) THEN CREATE POLICY "Anyone can view active subscription plans" ON public.subscription_plans FOR
SELECT
    USING (is_active = TRUE);
...
IF NOT EXISTS (
    ...
    WHERE
        ...
        AND policyname = 'Anyone can view supported currencies'
) THEN CREATE POLICY "Anyone can view supported currencies" ON public.supported_currencies FOR
SELECT
    USING (is_active = TRUE);
...
DO $$ BEGIN IF NOT EXISTS (
    ...
    FROM
        pg_trigger
    WHERE
        tgname = 'update_user_profiles_updated_at'
) THEN CREATE TRIGGER update_user_profiles_updated_at BEFORE
UPDATE
...
IF NOT EXISTS (
    ...
    FROM
        pg_trigger
    WHERE
        tgname = 'update_clothing_items_updated_at'
) THEN CREATE TRIGGER update_clothing_items_updated_at BEFORE
UPDATE
...
IF NOT EXISTS (
    ...
    FROM
        pg_trigger
    WHERE
        tgname = 'update_categories_updated_at'
) THEN CREATE TRIGGER update_categories_updated_at BEFORE
UPDATE
    ON public.clothing_categories FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
IF NOT EXISTS (
    ...
    FROM
        pg_trigger
    WHERE
        tgname = 'validate_user_currencies'
) THEN CREATE TRIGGER validate_user_currencies BEFORE
INSERT
    OR
UPDATE
    ON public.user_profiles FOR EACH ROW EXECUTE FUNCTION validate_currency_code();
